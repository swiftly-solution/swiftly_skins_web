...
export default function Pagination({ pages, page, firstPage, lastPage, setPage }: Props) {
    ...
    return (
        <PaginationBox className="select-none">
            {!firstPage && <div key={v4()} onClick={() => setPage((last) => last - 1)}>
            ...
            {pages.map((pg) => (
                <div key={pg} onClick={() => setPage(pg)} className={pg == page ? "current" : ""}>
                    {pg}
                </div>
            ))}
            {!lastPage && <div key={v4()} onClick={() => setPage((last) => last + 1)}>
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
