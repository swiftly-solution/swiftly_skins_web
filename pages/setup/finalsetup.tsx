...
export default function SetupFinal() {
    ...
    if (finished == true && error.length == 0) {
        ...
        else if (response != "finalsetup") Router.push(`/setup/${response}`);
    }
    if (error.length) console.log(error);

    return (
        <PageContentBlock title={t("setup.finalsetup.title")} loading={!finished}>
            {error.length > 0 ? error : <>
                <div className="flex flex-col ml-auto mr-auto gap-4">
                    <Box>
                        {t("setup.finalsetup.message")}
                    </Box>
                    <Box>
                        ...
                        }}>{t("setup.finalsetup.button")}</Button>
                    </Box>
                </div>
            </>}
        </PageContentBlock>
