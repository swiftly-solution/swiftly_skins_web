...
export default function Skins() {
    ...
    const userResponse = ExecuteRequest<string[]>("/api/user/fetchUserData", "post", { category: "skin", steamid: session.status == 'authenticated' ? GetSteamIDFromSession(session.data) : '-1' });

    return (
        <PageContentBlock title={t("skins.title")} loading={session.status == 'loading' || finished == false || (finished == true && response == false)}>
            {error.length > 0 ? error : (session.status == 'unauthenticated' ? <NotLoggedIn /> : <SelectPage data={FetchSkins()} userData={userResponse.response || []} />)}
        </PageContentBlock>
    )
}